...
namespace Ablaze
{
	...
	void Graphics::DrawString(float x, float y, const String& text, Font* font, const Color& color)
	{
		...
		obj->AddComponent(new MeshRenderer(new Mesh(font->CreateModel(text), Material(color, ResourceManager::DefaultFontShader(), "Tex0", font))));
	...
	void Graphics::DrawRectangle(float x, float y, float w, float h, const Material& material)
	{
		GameObject* obj = GameObject::Instantiate("Object", x, y, 0);
	...
	{
		DrawRectangle(x, y, w, h, Material(color, ResourceManager::DefaultColorShader()));
	}

	void Graphics::DrawImage(float x, float y, float w, float h, Texture2D* image)
	{
		DrawRectangle(x, y, w, h, Material(Color::White(), ResourceManager::DefaultTextureShader(), "Tex0", image));
	}
